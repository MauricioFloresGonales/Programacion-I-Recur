...
LinkedList* filter(LinkedList* this,int (*fn)(void*pElement))
{
...
int ll_map(LinkedList* this,int(*aleatorio)(void*pElement))
{
    void* carrera = NULL;
    int len;
    int i;
    int retorno = -1;
    if(this!=NULL && aleatorio!=NULL)
    {
        len = ll_len(this);
        for(i=0;i<len;i++)
        {
            carrera=ll_get(this,i);
            carrera = aleatorio(carrera);
            ll_set(this,i,carrera);
            retorno = 0;
        }
    }
    return retorno;
